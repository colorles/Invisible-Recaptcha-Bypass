#MADE BY COLORLES911 AKA C911 FOR TESTING PURPOSE , USE AT YOUR OWN RISK
import requests


def BypassInvisable(geturl,posturl)->str:
    V=str(geturl).split("v=")[1].split('&')[0].strip()
    CO=str(geturl).split("co=")[1].split('&')[0].strip()
    KEY=str(geturl).split("k=")[1].split('&')[0].strip()
    def retC()->str:
        return requests.get(geturl).text.split('id="recaptcha-token" value="')[1].split('"')[0].strip()
    c=retC()   
    def returnRespp()->str:
        origin2=str(geturl).split("https://")[1].split('/')[0]
        ...
        return requests.post(
            ...
            ).text.split('["rresp","')[1].split('"')[0].strip()
    return returnRespp()
